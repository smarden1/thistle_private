...
object Lookaround {
	...
	def lookBehind[T](n: Int, predicate: MatchPredicate[T]) =
		(m : MatchState[T]) =>
			lookAt(m.index - n, predicate)(m)

	// applies this given predicate and sees if it matches n places ahead
	def lookAhead[T](n: Int, predicate: MatchPredicate[T]) =
		(m : MatchState[T]) =>
			lookAt(m.index + n, predicate)(m)

	def next[T](predicate: MatchPredicate[T]) =
		lookAhead(1, predicate)

	def previous[T](predicate: MatchPredicate[T]) =
		lookBehind(1, predicate)

	//is slice a good name?
	def nextSlice[T](predicate: MatchPredicate[T]) =
		(m : MatchState[T]) => 
			lookaround(m.index, m.size, predicate)(m)

	def nextSlice[T](n:Int, predicate: MatchPredicate[T]) =
		(m : MatchState[T]) => 
			lookaround(m.index, m.index + n, predicate)(m)

	def previousSlice[T](n:Int, predicate: MatchPredicate[T]) =
		(m : MatchState[T]) => 
			lookaround(m.index - n, m.index, predicate)(m)

	def previousSlice[T](predicate: MatchPredicate[T]) =
		(m : MatchState[T]) => 
	...
	private def lookAt[T](n: Int, predicate: MatchPredicate[T]) =
		(m : MatchState[T]) => {
			m.series.size <= n &&
